...
inline Transition
mean_transition_state(const SType &state, long index,
...
inline MatrixXd transition_mat(const GRMDP<SType> &rmdp,
...
#pragma omp parallel for
  for (size_t s = 0; s < n; s++) {
    const Transition &&t =
        internal::mean_transition_state(states[s], s, policies);
...
inline numvec occfreq_mat(const GRMDP<SType> &rmdp, const Transition &init,
  ...
  const VectorXd initial_vec =
      Map<const VectorXd, Unaligned>(ivec.data(), ivec.size());

  // get transition matrix and construct (I - gamma * P^T)
  MatrixXd t_mat = MatrixXd::Identity(n, n) -
                   discount * transition_mat(rmdp, policies, true);
