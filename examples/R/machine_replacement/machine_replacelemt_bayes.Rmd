...
## Construct a biased policy to prefer going to the last state. This is to ensure that the "goal" state is sampled.

```{r}
last.stateid <- max(mdp.truth$idstatefrom, na.rm = TRUE)
num.state <- last.stateid + 1
...
ur.policy = data.frame(idstate = c(seq(0, last.stateid), seq(0, last.stateid)),
...
##  Solve Empirical MDP and Bayesian MDP
