...
# Generate dataframes from frequentist and Bayesian MDP
sample.model.revised <- function(baysian.posterior, mdp.freq, bayes.samples){
  num_states <- length(baysian.posterior$idstatefrom %>% unique())
  num_actions <- length(baysian.posterior$idaction %>% unique())
  ...
  for(s in 1:num_states){
    for(a in 1:num_actions){
      ...
      # The sample records would be of a 3 dimensional matrix: iter x num_states x num_actions
      # Compute an unique index to convert and store this as a 1 dimensional array.
      record.index <- stateFrom*num_actions + (action) + 1
      
      
      rewards <- rep(0.0, num_states)
      

      fci_nominal_trp <- rep(0, num_states)
      transition <- mdp.freq$mdp.nominal %>%
        filter(idstatefrom == stateFrom, idaction == action)

      probabilty <- transition %>% pull(probability)
      idstateto <- transition %>% pull(idstateto)

      # Add transitions  for the states
      for(i in 1:length(idstateto)){
        fci_nominal_trp[idstateto[i]+1] <- probabilty[i]
        rewards[idstateto[i]+1] <- transition$reward[i]
        
      }

      posterior.alpha <- rep(eps, num_states)
      posterior <- baysian.posterior %>%
        filter(idstatefrom == stateFrom, idaction == action)

      statesTo <- posterior$idstatesto[[1]]
      alpha <- posterior$posterior[[1]]



      for(i in 1:length(statesTo)){
        posterior.alpha[statesTo[i] + 1] <- alpha[i]
      }


      posteriori_samples <- rdirichlet(bayes.samples , alpha = posterior.alpha)
      bayes_nominal_trp <- colMeans(posteriori_samples)
      record.data[[record.index]] <- list("posteriori_samples"=posteriori_samples,
                                          "fci_nominal_trp"=fci_nominal_trp,
                                          "bayes_nominal_trp"=bayes_nominal_trp,
...
get.nonrobust.sol <- function(num_states, num_actions, discount_factor, sampled.model){
  ...
  sol.fmdp <- solve_mdp(fmdp, discount_factor, show_progress = FALSE, algorithm="pi")$valuefunction$value
  sol.bmdp <- solve_mdp(bmdp, discount_factor, show_progress = FALSE, algorithm="pi")$valuefunction$value
...
get.value.functions <- function(mdp.freq, mdp.bayesian, discount){
  
  sol.bayes <- rsolve_mdpo_sa(mdp.bayesian, discount, "exp", NULL, show_progress = FALSE)
  value.bayes <- sol.bayes$valuefunction$value
  
  sol.empirical <- solve_mdp(mdp.freq , discount, show_progress = FALSE)
  value.freq <- sol.empirical$valuefunction$value
  li.value.functions <- list()
  for(m in methods){
    if(grepl("BCI", method_names[m]))
      li.value.functions[m] <- list( value.bayes )
    else
      li.value.functions[m] <- list( value.freq )
  }
  return(li.value.functions)
